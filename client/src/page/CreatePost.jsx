...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://mern-ai-image.onrender.com/api/v1/dalle', {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://mern-ai-image.onrender.com/api/v1/post', {
    ...
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[14px] max-w-[500px]">
          Gere uma imagem imaginativa através da DALL-E AI e compartilhe-a aqui.
        </p>
      </div>
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          <p className="mt-2 text-[#666e75] text-[14px]">** Depois de criar a imagem desejada, você pode 
